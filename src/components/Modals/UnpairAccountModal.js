...
export default function UnpairAccountModal(props) {
  ...
  const onConfirm = async () => {
    try {
      setLoading(true);
      const resp = await API('user/auth/unpair', 'POST');
      ...
      setLoading(false);
      console.log('error unpair', error);
    } finally {
      setLoading(false);
    }
  };
    ...
    <Modal
      ...
      <View
        ...
        <Text
          ...
          {
            'Your basic contact details are still saved, so can still log in to the app and try pairing again, if and whenever you’d want to experience Closer again with a new partner.'
          }
        </Text>
