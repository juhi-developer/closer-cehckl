...
export default function Moments(props) {
  ...
  const showInAppReview = async () => {
    ...
    console.log('checkInAppReview', checkInAppReview);
  ...
  const toCheckVersion = async () => {
    try {
      ...
      console.log('ressssssss version', res);
      ...
      console.log('localBuildNumber', localBuildNumber);
      console.log('androidVersionCode', androidVersionCode);
      console.log('iosVersioncode', iosVersioncode);
      console.log('localVersionCode', localVersionCode);
  ...
  const updateFcmToken = async () => {
    ...
    console.log('update token', Platform.OS, updateToken);
  };
  ...
  React.useEffect(() => {
    const subscription = RNShake.addListener(async () => {
      if (
        ...
        setLoading(true);
        console.log('horny mode rn shake', Platform.OS);
        HapticFeedbackHeavy();
  ...
  const handlePairingTimeActions = newData => {
    try {
      const pairingTime = newData?.user?.partnerData?.pairingTime;
      console.log('pairingTime', pairingTime);
  ...
  const performTimedActions = (hoursPassed, newData) => {
    try {
      ...
      // Check pairing status if toolTipCurrentState is '' and partner ID is defined
      console.log('newData?.user?.toolTip', newData?.user?.toolTip);
      console.log('toolTipCurrentState', toolTipCurrentState);
      console.log('partner data', newData?.user?.partnerData?.partner?._id);
      if (
  ...
  useEffect(() => {
    const checkApiAndExecute = () => {
      if (!initialApi && pendingAction !== null) {
        console.log('API data is ready. Executing action:', pendingAction);
        setTimeout(() => {
    ...
    const notificationScroll = EventRegister.addEventListener(
      ...
      data => {
        console.log('Received notification scroll event:', data);
        if (!initialApi) {
          scrollHandler(data);
        } else {
          console.log('API not ready, setting pending action');
  ...
  useEffect(() => {
    if (isSocketConnected && socket) {
      ...
      socket.on('personaliseMismatch', data => {
        console.log('personalise mismatch', data);
      ...
      socket.on('unpairPartner', data => {
        ...
        console.log('partner unpaired', data);
      });
      ...
      socket.on('optionsHorny', data => {
        console.log('option horny sokcet', data);
        setTimeout(async () => {
        ...
        setLoading(false);
        console.log('dta optionQnA ', data);
      });
  ...
  const setStoriesData = async newData => {
    ...
    console.log('local stories', localStories);
    console.log('new stories', newData);
    ...
    newData.forEach(newStory => {
      ...
      console.log('local story index', localStoryIndex);
  ...
  const UploadStoriesHandler = async (imageSet, type, captionAdded) => {
    ...
    try {
      ...
      try {
        ...
        if (resp.body.statusCode === 200) {
          console.log('photo type', type, captionAdded);
                        ...
                        <>
                          {profileData !== undefined &&
                            ...
                            (hornyMode
                              ...
                              : profileData?.QnA?.question !== undefined) && (
                              <View
                                pointerEvents={
